...
case "$(uname -s)" in
    Linux)
        ...
        if command -v yum &>/dev/null; then
            info "📦 使用 yum 作为包管理器 (CentOS/RHEL)."

            YUM_PACKAGES=("curl" "file" "git236" "jq" "vim" "tmux" "tig" "ctags")

            info "安装 EPEL 仓库和基础依赖..."
            sudo yum install -y epel-release

            info "尝试从 yum 仓库安装软件包: ${YUM_PACKAGES[*]}"
        ...
        install_from_github "fastfetch" "fastfetch-cli/fastfetch" "fastfetch-linux-$(get_arch_name fastfetch)-polyfilled.tar.gz$" --strip-components=3 "fastfetch-linux-$(get_arch_name fastfetch)-polyfilled/usr/bin/fastfetch"
